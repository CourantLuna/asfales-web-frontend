      ...
      <table className="min-w-full text-sm text-left border-collapse">
        <thead>
          <tr className="border-b">
            <th className="text-muted-foreground font-medium w-48"></th>
        ...
        <tbody>
           {rows.map((row, rowIndex) => {
            ...
            if (row.rowType === "calificacion") {
              return (
                <tr key={rowIndex} className="border-b">
                  <td className="py-5 text-muted-foreground">{row?.label}</td>
                  {row.values.map((val, i) => (
         ...
         if (row.rowType === "beneficios") {
              ...
              return (
                <tr key={rowIndex} className="border-b">
                  <td className="py-3 text-muted-foreground align-top">{row?.label}</td>
                  {row.values.map((val, i) => (
            ...
            if (row.rowType === "acciones") {
              return (
                <tr key={rowIndex} className="h-[20px]">
                  <td className="py-4 text-muted-foreground">{row?.label}</td>
                  {row.values.map((actions, i) => (
                    <td key={i} className="text-center space-x-2 mt-5 pt-4">
